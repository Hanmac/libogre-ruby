...
/*:nodoc:
*/
VALUE OgreTextureUnitState_get_alpha(VALUE self)
{
	return _self->getIsAlpha() ? Qtrue : Qfalse;
}
/*:nodoc:
*/
VALUE OgreTextureUnitState_set_alpha(VALUE self,VALUE val)
{
	_self->setIsAlpha(RTEST(val));
	return val;
}
/*:nodoc:
*/
...
void Init_OgreTextureUnitState(VALUE rb_mOgre)
{
	...
	rb_define_method(rb_cOgreTextureUnitState,"alpha",RUBY_METHOD_FUNC(OgreTextureUnitState_get_alpha),0);
